...
const ProductCard = ({image,title,price,oldPrice,discount,status}) => {
  return (
    <div className='bg-white p-4 rounded-lg shadow hover:shadow-lg transition relative overflow-hidden'>
      {discount && <div className='absolute top-2 left-2 bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded'> -{discount}% </div>}
      <img src={image} alt="image" className='w-full h-40 object-cover rounded' />
      ...
      {status==='soldOUt'?(
        ...
        <p className='mt-4 w-full bg-green-600 hover:bg-green-700 text-center text-white py-5 px-5 rounded text-sm'>Shop Now</p>
      )}
    </div>
  );
};
